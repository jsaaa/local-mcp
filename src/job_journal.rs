...
pub fn finish(session_id: &str, job_id: Uuid, result: &Result<String>) -> Result<JobRecord> {
    ...
    match result {
        Ok(text) => {
            record.state = result_state(text);
...
fn result_state(text: &str) -> JobState {
    match serde_json::from_str::<Value>(text)
        .ok()
        .and_then(|value| value.get("termination")?.as_str().map(str::to_owned))
        .as_deref()
    {
        Some("exited") | None => JobState::Completed,
        Some(_) => JobState::Stopped,
    }
}
...
mod tests {
    ...
    #[test]
    fn lifecycle_termination_is_persisted_as_stopped_with_its_result() {
        let session_id = format!("journal-lifecycle-stop-{}", Uuid::new_v4());
        let job_id = Uuid::new_v4();
        create_running(
            &session_id,
            job_id,
        ...
        let result = Ok(serde_json::json!({
            "exit_code": -1,
            "termination": "stopped",
            "termination_trigger": "stop",
            "stdout_head": "partial"
        })
        .to_string());

        let record = finish(&session_id, job_id, &result).unwrap();
        assert_eq!(record.state, JobState::Stopped);
        assert_eq!(record.exit_code, Some(-1));
        assert!(record.result.as_deref().unwrap().contains("termination"));
